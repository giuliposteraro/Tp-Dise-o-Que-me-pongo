...
public class Atuendo {
	...
	private Prenda obtenerPrendaDeCategoria(ECategoria categoria, List<Prenda> conjuntoDePrendas) {
		return conjuntoDePrendas.stream().filter(prenda -> categoria.equals(prenda.getCategoria())).findFirst()
				.orElse(null);
	}
	...
	public List<Double> getNivelesAbrigo() {
		return Arrays.asList(abrigo.getNivelAbrigo() + superior.getNivelAbrigo(), inferior.getNivelAbrigo(),
				calzado.getNivelAbrigo(), accesorio.getNivelAbrigo());
	}
